...
proc freq data=regal.al_al; tables 
new_diagname
new_vidleyname
new_diagfabname new_diagmkbname
new_etnosname 
 new_factorriskname 
new_sibling
; run;
...
proc freq data=regal.al_pt; tables 
gendercodename
new_cityname
new_consentname
new_deathreasonname
...
proc sort data=regal.al_al out=pat1; by new_contact; run;
data pat11; set pat1; contactid=new_contact; run;
proc sort data=regal.al_pt out=pat2; by contactid; run;
data patall; merge pat11 pat2; by contactid; 
if new_diagnosisage<10 then new_diagnosisage=.;
if new_lpuname='ФГБУ ГНЦ МЗСЦ РФ' then center='ГНЦ';
else center='регионы';

run;
proc means data=patall n median min max mean; var new_diagnosisage; run;
proc means data=patall n median min max mean; var new_diagnosisage; class center; run;
/*

proc freq data=patall; 
tables  (new_patientstatname new_diagname new_diagfabname new_diagmkb new_diagmkbname new_etnosname )*center/nopercent norow;run;
proc print data=regal.al_ev label; var new_contactname new_event new_eventname new_event_date new_event_txt ; run;
*/



proc sort data=regal.al_ev out=ev1; by new_contact; run;
data ev2; set ev1; by new_contact;
retain 
datdeath datlcont datrel alive rel;
...
label
datdeath='дата смерти' datlcont='дата п.контакта' datrel='дата рецидива' alive='жив?' rel='рецидив?'
;
/*
proc freq data=ev2; tables alive rel; run;
*/
data patalev; merge patall (in=inp) ev2 (in=ine); by  contactid; 



ip=inp; ie=ine; 
run;
proc freq data=patalev; tables ip*ie; run;

proc freq data=patalev; tables new_patientstat*alive; run;

data aa; set patalev; 
d1=datlcont-new_datelastcontact;
d2=datdeath-new_deathdate;
run;
proc print; var d1 d2; run;
